defmodule Malachi.PartitionManagerTest do
  use ExUnit.Case, async: true

  describe "get_partition/1" do
    test "returns consistent partition for same queue name" do
      {_name, partition1} = Malachi.PartitionManager.get_partition("test_queue")
      {_name, partition2} = Malachi.PartitionManager.get_partition("test_queue")

      assert partition1 == partition2
    end

    test "distributes different queues across partitions" do
      partitions =
        for i <- 1..100 do
          {_name, partition} = Malachi.PartitionManager.get_partition("queue_#{i}")
          partition
        end

      unique_partitions = Enum.uniq(partitions)
      assert length(unique_partitions) > 1
    end

    test "returns integer partition number" do
      {_name, partition} = Malachi.PartitionManager.get_partition("test")
      assert is_integer(partition)
      assert partition >= 0
    end
  end

  describe "get_partition_count/0" do
    test "returns total number of partitions" do
      count = Malachi.PartitionManager.get_partition_count()
      assert is_integer(count)
      assert count > 0
    end

    test "partition count matches expected calculation" do
      schedulers = System.schedulers_online()
      multiplier = Application.get_env(:malachi, :partition_multiplier, 100)
      expected = schedulers * multiplier

      assert Malachi.PartitionManager.get_partition_count() == expected
    end
  end
end
