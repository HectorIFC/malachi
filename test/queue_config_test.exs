defmodule MalachiMQ.QueueConfigTest do
  ...
  describe "list_queues/0" do
    ...
    test "returns sorted list by created_at" do
      ...
      {:ok, _config1} = MalachiMQ.QueueConfig.create_queue(queue1)
      :timer.sleep(10)
      {:ok, _config2} = MalachiMQ.QueueConfig.create_queue(queue2)
  ...
  describe "queue_exists?/1" do
    test "returns true for existing queue", %{queue_name: queue_name} do
      {:ok, _config} = MalachiMQ.QueueConfig.create_queue(queue_name)
      assert MalachiMQ.QueueConfig.queue_exists?(queue_name)
    end

    test "returns false for non-existent queue" do
      refute MalachiMQ.QueueConfig.queue_exists?("non_existent_queue_#{:rand.uniform(1_000_000)}")
    end
  end
  ...
  describe "create_queue/2 with backpressure fields" do
    test "creates queue with custom max_buffer_size", %{queue_name: queue_name} do
      {:ok, config} = MalachiMQ.QueueConfig.create_queue(queue_name, max_buffer_size: 50_000)
      assert config.max_buffer_size == 50_000
    end

    test "creates queue with custom overflow_behavior", %{queue_name: queue_name} do
      {:ok, config} = MalachiMQ.QueueConfig.create_queue(queue_name, overflow_behavior: :drop_oldest)
    ...
    test "returns error for invalid overflow_behavior", %{queue_name: queue_name} do
      {:error, :invalid_overflow_behavior, _msg} =
        MalachiMQ.QueueConfig.create_queue(queue_name, overflow_behavior: :invalid_strategy)
    end
  ...
  describe "update_queue/3 - safe updates" do
    test "updates queue with no buffer size change", %{queue_name: queue_name} do
      {:ok, _} = MalachiMQ.QueueConfig.create_queue(queue_name, max_buffer_size: 1000)

      {:ok, :updated} =
        MalachiMQ.QueueConfig.update_queue(queue_name, %{
    ...
    test "updates with buffer size increase (always safe)", %{queue_name: queue_name} do
      ...
      {:ok, :updated} = MalachiMQ.QueueConfig.update_queue(queue_name, %{max_buffer_size: 2000})
    ...
    test "returns error for non-existent queue" do
      {:error, :queue_not_found} =
        MalachiMQ.QueueConfig.update_queue("nonexistent_#{:rand.uniform(1_000_000)}", %{max_buffer_size: 1000})
    end
    ...
    test "returns error for invalid overflow_behavior", %{queue_name: queue_name} do
      ...
      {:error, :invalid_overflow_behavior, _} =
        MalachiMQ.QueueConfig.update_queue(queue_name, %{overflow_behavior: :invalid_strategy})
    end
  end
  ...
  describe "update_queue/3 - buffer size reduction with excess" do
    test "allows update with no excess (buffer empty)", %{queue_name: queue_name} do
      ...
      {:ok, :updated} = MalachiMQ.QueueConfig.update_queue(queue_name, %{max_buffer_size: 500})
    ...
    test "allows update with small excess (within 50% threshold)", %{queue_name: queue_name} do
      ...
      {:ok, :updated_with_warning, warning} =
        MalachiMQ.QueueConfig.update_queue(queue_name, %{max_buffer_size: 500})
    ...
    test "allows forced update with large excess", %{queue_name: queue_name} do
      ...
      {:ok, :forced_update, result} =
        MalachiMQ.QueueConfig.update_queue(queue_name, %{max_buffer_size: 500}, force: true)
  ...
  describe "update_queue/3 - edge cases" do
    test "handles exactly at 50% threshold", %{queue_name: queue_name} do
      ...
      result = MalachiMQ.QueueConfig.update_queue(queue_name, %{max_buffer_size: 500})
