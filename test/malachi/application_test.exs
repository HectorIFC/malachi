defmodule Malachi.ApplicationTest do
  ...
  # boot evidence pass because a test ran rather than because the node booted.
  defp no_start, do: fn -> {:ok, {:not_started, node()}} end

  # A flag store holding exactly `flags`, built through the machine's own command so the test and the
  # code agree on the shape.
  defp store(flags) do
