defmodule Malachi.LogTest do
  ...
  defp open(dir), do: Log.open(dir, max_bytes: 120, index_interval: 32)
  ...
  defp read_all(log, offset, acc) do
    case Log.read(log, offset, 100) do
      ...
      {:ok, recs} -> read_all(log, offset + length(recs), [recs | acc])
    end
  end
  ...
  describe "rolling across segments" do
    test "appending past max_bytes creates multiple segments and reads span them",
      ...
      assert length(Path.wildcard(Path.join(dir, "*.log"))) >= 2
    ...
    test "reads from a sealed segment use the persisted index", %{tmp_dir: dir} do
      {:ok, log} = open(dir)
      ...
      assert {:ok, [r]} = Log.read(log, first_base, 1)
      assert r.offset == first_base

      :ok = Log.close(log)
    end

    test "explicit roll seals the active segment", %{tmp_dir: dir} do
      {:ok, log} = Log.open(dir)
  ...
  describe "read bounds" do
    test "eof past the end and out_of_range below the start", %{tmp_dir: dir} do
      {:ok, log} = Log.open(dir, base_offset: 50)
      log = append_sync(log, rec("only"))

      assert {:ok, [r]} = Log.read(log, 50, 10)
      assert r.offset == 50
      assert Log.read(log, 51, 10) == :eof
      assert Log.read(log, 49, 10) == {:error, :out_of_range}
    end

    test "read on an empty log is eof", %{tmp_dir: dir} do
      {:ok, log} = Log.open(dir)
  ...
  describe "recovery" do
    test "recovers sealed + active segments and resumes appending", %{tmp_dir: dir} do
      {:ok, log} = open(dir)
      log = Enum.reduce(0..9, log, fn i, acc -> append_sync(acc, rec("v#{i}")) end)
      ...
      {:ok, log2} = Log.recover(dir, max_bytes: 120, index_interval: 32)
      ...
      assert {:ok, [r]} = Log.read(log2, next, 1)
      assert r.value == "after"

      :ok = Log.close(log2)
    end

    test "recovers a log whose last segment was sealed", %{tmp_dir: dir} do
      {:ok, log} = Log.open(dir)
      {:ok, log, _, _} = Log.append(log, [rec("a"), rec("b")])
      {:ok, log} = Log.sync(log)
      {:ok, log} = Log.roll(log)
    ...
    test "recovers an empty directory as a fresh log", %{tmp_dir: dir} do
      {:ok, log} = Log.recover(dir, base_offset: 7)
      assert log.next_offset == 7
      assert log.sealed_base_offsets == []
      assert Log.read(log, 7, 10) == :eof
    end

    test "recovery drops a partial trailing write in the active segment", %{tmp_dir: dir} do
      {:ok, log} = open(dir)
      log = Enum.reduce(0..9, log, fn i, acc -> append_sync(acc, rec("v#{i}")) end)
      :ok = Log.close(log)
      ...
      active_path =
        dir |> Path.join("*.log") |> Path.wildcard() |> Enum.sort() |> List.last()

      size = File.stat!(active_path).size
      {:ok, fd} = :file.open(active_path, [:read, :write, :raw, :binary])
  ...
  describe "segment naming" do
    test "segment files are named by zero-padded base offset", %{tmp_dir: dir} do
      {:ok, log} = Log.open(dir)
      log = append_sync(log, rec("x"))
      assert File.exists?(Path.join(dir, "00000000000000000000.log"))
      # sanity: Segment derives the same path
      seg = Segment.new("00000000000000000000", dir, base_offset: 0)
      assert File.exists?(Segment.path(seg))
      :ok = Log.close(log)
    end
  end
end
