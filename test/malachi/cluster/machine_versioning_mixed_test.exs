defmodule Malachi.Cluster.MachineVersioningMixedTest do
  ...
  `Malachi.Test.VersionedMetadataMachine` implements version 2, which adds `{:probe, topic}`; a peer
  pinned to 1 plays a binary that has never heard of it. Before the gate, the members that knew
  `:probe` applied it and the others skipped it, and the group's replicas held different states with no
  error anywhere. With the gate, `ra`'s `all` strategy keeps the effective version at 1 while any
  member lacks 2, and every member refuses `:probe` the same way.
  ...
  test "a member one version behind holds the group back, and every member refuses the newer command alike" do
    group = start_group([nil, nil, 1])
    barrier(group)
    await_effective(group, 1)

    # Several leader ticks later, the member without version 2 still holds the group at 1.
    Process.sleep(2_500)
    await_effective(group, 1)
  ...
  test "a full restart replays every member to the same state and effective version" do
    ...
    for index <- 0..2, do: restart(group, index, if(index == 2, do: 1, else: nil))
  ...
  test "a pinned rollout can roll back, finalizing lifts the version, and a rollback below it stops that member" do
    group = start_group([1, 1, 1])
    barrier(group)
    await_effective(group, 1)
    assert {:ok, {:error, _refused}} = command(group, {:probe, "p"})

    # Rollback in the middle of a pinned rollout: the member comes back on version-1 code and keeps up.
    rolled_back = restart(group, 0, 1)
    marker = barrier(group)
    ...
    # group reaches version 2 once the last member advertises it, with no election needed.
    for index <- 0..2 do
      restart(group, index, nil)
      barrier(group)
    end

    await_effective(group, 2)
    assert {:ok, {:ok, _root}} = command(group, {:probe, "p"})
    barrier(group)
    for state <- states(group), do: assert(Metadata.get_topic(state, "p"))

    # A rollback below the floor: the member meets the version-2 noop in its log and stops applying.
    stuck = restart(group, 2, 1)
    assert RaPeers.eventually(fn -> match?({{:stuck, 2, 1}, :stuck}, check(group, stuck)) end)
