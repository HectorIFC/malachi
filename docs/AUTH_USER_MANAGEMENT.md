...
because the default is a temp directory that is lost when it does not survive a restart (a recreated
container or a reboot). Brute-force protection is cluster-wide
...
re-logged whenever that directory does not survive (a recreated container or a reboot).
