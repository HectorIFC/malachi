...
No default credentials ship. If you have not set `MALACHI_ADMIN_PASS`, Malachi **generates a random admin password and logs it**: save it from the logs (it cannot be recovered). This is a one-time event only when `MALACHI_RA_DATA_DIR` points at a persistent volume; on the temp default the store is ephemeral, so a new password is generated and logged on every restart. Provide your own with `MALACHI_ADMIN_PASS` (and `MALACHI_PRODUCER_PASS` / `MALACHI_CONSUMER_PASS` / `MALACHI_APP_PASS` for the other service accounts), or set `MALACHI_DISABLE_DEFAULT_USERS=true` to seed nothing and manage users yourself.
