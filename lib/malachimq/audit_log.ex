defmodule MalachiMQ.AuditLog do
  @moduledoc """
  ...
  @table_main :malachimq_audit_log
  @table_by_type :malachimq_audit_by_type
  @table_by_user :malachimq_audit_by_user
  # 1 hour
  ...
  def init(_opts) do
    ...
    output_mode = Application.get_env(:malachimq, :audit_log_output, :ets_only)
    file_path = Application.get_env(:malachimq, :audit_log_file)
    max_size_mb = Application.get_env(:malachimq, :audit_log_max_size_mb, 1)
    ...
    Logger.info(MalachiMQ.I18n.t(:audit_log_started, %{retention_days: @retention_days}))
  ...
  def handle_cast({:log_event, event_type, context, action, status, metadata}, state) do
    ...
    try do
      MalachiMQ.Metrics.increment_audit_event(event_type)
    rescue
