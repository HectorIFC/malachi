defmodule MalachiMQ.TCPProtocol do
  @moduledoc """
  Implements the MalachiMQ TCP/JSON protocol.
  ...
  defp handle_action(
    ...
    with_permission(session, :produce, socket, transport, fn ->
      ...
      publish_limit = Application.get_env(:malachimq, :publish_rate_limit, 1_000)
      publish_window_ms = Application.get_env(:malachimq, :publish_rate_window_ms, 1_000)

      case MalachiMQ.RateLimiter.check_limit(session.username, :publish, %{
             limit: publish_limit,
        ...
        :ok ->
          ...
          with :ok <- MalachiMQ.Validator.validate_message_size(p, max_size),
               :ok <- MalachiMQ.Validator.validate_queue_name(q),
               :ok <- MalachiMQ.Validator.validate_payload(p),
               :ok <- MalachiMQ.Validator.validate_headers(h) do
            case MalachiMQ.Queue.enqueue(q, p, h) do
              {:ok, _message} ->
        ...
        {:error, :rate_limit_exceeded, retry_after_ms} ->
          MalachiMQ.Metrics.increment_rate_limit_blocked(:publish)
  ...
  defp handle_action(socket, %{"action" => "subscribe", "queue_name" => q}, session, transport, _client_ip) do
    with_permission(session, :consume, socket, transport, fn ->
      # Validate queue name
      case MalachiMQ.Validator.validate_queue_name(q) do
        :ok ->
          # Check subscribe rate limit
          subscribe_limit = Application.get_env(:malachimq, :subscribe_rate_limit, 100)
          subscribe_window_ms = Application.get_env(:malachimq, :subscribe_rate_window_ms, 60_000)

          case MalachiMQ.RateLimiter.check_limit(session.username, :subscribe, %{
            ...
            {:error, :rate_limit_exceeded, retry_after_ms} ->
              MalachiMQ.Metrics.increment_rate_limit_blocked(:subscribe)
              send_error(socket, :rate_limit_exceeded, %{"retry_after_ms" => retry_after_ms}, transport)
              :ok
          end
  ...
  defp handle_action(socket, %{"action" => "get_queue_info", "queue_name" => q}, session, transport, _client_ip) do
    ...
    if has_any_permission?(session, [:admin, :produce, :consume]) do
      ...
      {:ok, pressure_info} = MalachiMQ.Backpressure.get_queue_pressure(q)
      ...
      response_data = %{
        "queue_info" => %{
          ...
          "stats" => %{
            ...
            "backpressure_active" => MalachiMQ.Backpressure.should_apply_backpressure?(pressure_info)
          }
        }
      }
  ...
  defp handle_action(
    ...
    with_permission(session, :produce, socket, transport, fn ->
      ...
      # Validate inputs
      with :ok <- MalachiMQ.Validator.validate_channel_name(ch),
           :ok <- MalachiMQ.Validator.validate_payload(p),
           :ok <- MalachiMQ.Validator.validate_headers(h) do
  ...
  defp handle_action(socket, %{"action" => "channel_subscribe", "channel_name" => ch}, session, transport, _client_ip) do
    with_permission(session, :consume, socket, transport, fn ->
      ...
      case MalachiMQ.Validator.validate_channel_name(ch) do
        :ok ->
    ...
    with_permission(session, :admin, socket, transport, fn ->
      ...
      case parse_overflow_behavior(Map.get(msg, "overflow_behavior")) do
        {:ok, updates} ->
          ...
          case MalachiMQ.QueueConfig.update_queue(q, updates, force: force) do
            {:ok, :updated} ->
  ...
  defp handle_action(socket, %{"action" => "get_queue_config", "queue_name" => q}, session, transport, _client_ip) do
    with_permission(session, :admin, socket, transport, fn ->
      case MalachiMQ.QueueConfig.queue_exists?(q) do
        true ->
          config = MalachiMQ.QueueConfig.get_config(q)
          stats = MalachiMQ.Queue.get_stats(q)
          {:ok, pressure_info} = MalachiMQ.Backpressure.get_queue_pressure(q)
  ...
  defp with_permission(session, permission, socket, transport, action_fn) do
    if MalachiMQ.Auth.has_permission?(session.permissions, permission) do
      action_fn.()
    else
  ...
  defp has_any_permission?(session, permissions) do
    Enum.any?(permissions, fn perm ->
      MalachiMQ.Auth.has_permission?(session.permissions, perm)
    end)
  end
  ...
  defp build_backpressure_response(queue_name) do
    {:ok, pressure_info} = MalachiMQ.Backpressure.get_queue_pressure(queue_name)
    backpressure = MalachiMQ.Backpressure.should_apply_backpressure?(pressure_info)
