defmodule MalachiMQ.Metrics do
  ...
  def get_metrics(queue_name) do
    ...
    backpressure_status =
      if Code.ensure_loaded?(MalachiMQ.Backpressure) do
        MalachiMQ.Backpressure.get_pressure_status(queue_name)
      else
        :low_pressure
      end
  ...
  def get_system_metrics do
    ...
    %{
      ...
      tls: %{
        enabled: Application.get_env(:malachimq, :enable_tls, false),
        required: Application.get_env(:malachimq, :require_tls, false),
  ...
  defp get_atom_monitor_stats do
    if Code.ensure_loaded?(MalachiMQ.AtomMonitor) and Process.whereis(MalachiMQ.AtomMonitor) do
      try do
        MalachiMQ.AtomMonitor.get_stats()
      rescue
  ...
  defp get_memory_monitor_stats do
    if Code.ensure_loaded?(MalachiMQ.MemoryMonitor) and Process.whereis(MalachiMQ.MemoryMonitor) do
      try do
        MalachiMQ.MemoryMonitor.get_memory_stats()
      rescue
        _ -> %{}
      end
    else
