defmodule MalachiMQ.Queue do
  ...
  @overflow_log_table :malachimq_overflow_logs
  ...
  def enqueue(queue_name, payload, headers \\ %{}) do
    with :ok <- MalachiMQ.Validator.validate_queue_name(queue_name) do
  ...
  def get_stats(queue_name) do
    case MalachiMQ.Validator.validate_queue_name(queue_name) do
      :ok ->
        {name, partition} = MalachiMQ.PartitionManager.get_partition(queue_name)
  ...
  defp apply_overflow_behavior(message, from, state, config, buffer_size) do
    case config.overflow_behavior do
      :drop_newest ->
        ...
        MalachiMQ.Metrics.increment_dropped(message.queue)
      ...
      :reject ->
        ...
        MalachiMQ.Metrics.increment_rejected(message.queue)
      ...
      :block ->
        ...
        if blocked_count >= config.max_blocked_producers do
          ...
          # Update metrics
          MalachiMQ.Metrics.set_blocked_producers_count(message.queue, map_size(new_lookup))
          MalachiMQ.Metrics.increment_total_producers_blocked(message.queue)
  ...
  def handle_info({:block_timeout, from, ref}, state) do
    ...
    case Map.get(state.blocked_producers.lookup, from) do
      {_message, ^ref, _timer_ref} ->
        ...
        # Update metrics
        MalachiMQ.Metrics.set_blocked_producers_count(state.name, map_size(new_lookup))
  ...
  def handle_info(:maybe_unblock_producers, state) do
    ...
    config = MalachiMQ.QueueConfig.get_config(state.name)
  ...
  defp do_log_overflow(queue_name, event_type, current, max) do
    Logger.warning(
      MalachiMQ.I18n.t(:queue_overflow_event,
        queue: queue_name,
        event: event_type,
        current: current,
        max: max
