defmodule MalachiMQ.Dashboard.SecurityHeaders do
  @moduledoc """
  ...
  def build_csp_header do
    csp =
      Application.get_env(:malachimq, :dashboard_csp) ||
  ...
  def build_hsts_header do
    tls_enabled = Application.get_env(:malachimq, :enable_tls, false)
    hsts_enabled = Application.get_env(:malachimq, :hsts_enabled, true)

    if tls_enabled and hsts_enabled do
      max_age = Application.get_env(:malachimq, :hsts_max_age, 31_536_000)
      include_subdomains = Application.get_env(:malachimq, :hsts_include_subdomains, true)
  ...
  def build_cors_headers(request_path) do
    ...
    if request_path in ["/metrics", "/stream"] do
      cors_enabled = Application.get_env(:malachimq, :dashboard_cors_enabled, false)

      if cors_enabled do
        origins = Application.get_env(:malachimq, :dashboard_cors_origins, ["*"])
