defmodule MalachiMQ.TCPAcceptor do
        ...
        client_socket =
          if transport == :ssl do
            case :ssl.handshake(client, timeout) do
              ...
              {:error, reason} ->
                MalachiMQ.Metrics.increment_tls_handshake_failed()
  ...
  defp handle_client(socket, transport) do
    ...
    case MalachiMQ.ConnectionLimiter.register_connection(self(), client_ip) do
      :ok ->
        ...
        after
          ...
          MalachiMQ.ConnectionLimiter.unregister_connection(self())
        end
  ...
  defp validate_and_authenticate(
    ...
    case LockoutManager.locked?(username, client_ip) do
      {:locked, time_remaining_ms} ->
        ...
        MalachiMQ.Metrics.increment_account_lockout_blocked()
      ...
      :not_locked ->
        ...
        auth_limit = Application.get_env(:malachimq, :auth_rate_limit, 10)
        auth_window_ms = Application.get_env(:malachimq, :auth_rate_window_ms, 60_000)

        case MalachiMQ.RateLimiter.check_limit(client_ip, :auth, %{limit: auth_limit, window_ms: auth_window_ms}) do
          :ok ->
            # STEP 3: Perform authentication
            case MalachiMQ.Auth.authenticate(username, password, client_ip) do
              {:ok, token} ->
  ...
  defp validate_token_and_respond(token, state, client_ip) do
    ...
    case MalachiMQ.Auth.validate_token(token, client_ip) do
      {:ok, session} ->
