defmodule MalachiMQ.Dashboard do
  use GenServer
  require Logger
  alias MalachiMQ.AuditLog
  alias MalachiMQ.Auth
  alias MalachiMQ.ConnectionRegistry
  alias MalachiMQ.Dashboard.SecurityHeaders
  ...
  defp handle_route_with_auth(socket, request, headers, client_ip) do
    auth_enabled = Application.get_env(:malachimq, :dashboard_auth_enabled, true)
      ...
      cookie_string ->
        ...
        |> Enum.find_value(fn cookie ->
          case cookie |> String.trim() |> String.split("=", parts: 2) do
            ["malachimq_token", value] -> String.trim(value)
            _ -> nil
          end
        end)
    end
  ...
  defp validate_token_with_rate_limit(token, client_ip, path) do
    ...
    rate_limit = Application.get_env(:malachimq, :dashboard_auth_rate_limit, 10)
    rate_window = Application.get_env(:malachimq, :dashboard_auth_rate_window_ms, 60_000)
  ...
  defp has_required_permission?(permissions, path) do
    require_admin = Application.get_env(:malachimq, :dashboard_require_admin_for_html, true)
  ...
  defp send_login_success(socket, token) do
    ...
    secure_flag =
      if Application.get_env(:malachimq, :enable_tls), do: "; Secure", else: ""

    cookie_header =
      "Set-Cookie: malachimq_token=#{token}; HttpOnly; Path=/; SameSite=Strict#{secure_flag}"
  ...
  defp send_auth_required(socket, path) do
    ...
    WWW-Authenticate: Bearer realm="MalachiMQ Dashboard"\r
    Content-Type: application/json\r
  ...
  defp serve_rate_limits(socket) do
    rate_limits = %{
      enabled: Application.get_env(:malachimq, :rate_limit_enabled, true),
      top_blocked: %{
      ...
      config: %{
        auth: %{
          limit: Application.get_env(:malachimq, :auth_rate_limit, 10),
          window_ms: Application.get_env(:malachimq, :auth_rate_window_ms, 60_000)
        },
        publish: %{
          limit: Application.get_env(:malachimq, :publish_rate_limit, 1_000),
          window_ms: Application.get_env(:malachimq, :publish_rate_window_ms, 1_000)
        },
        subscribe: %{
          limit: Application.get_env(:malachimq, :subscribe_rate_limit, 100),
          window_ms: Application.get_env(:malachimq, :subscribe_rate_window_ms, 60_000)
        }
      }
    }
  ...
  defp serve_logout(socket, headers) do
    ...
    response =
      "HTTP/1.1 302 Found\r\nLocation: /login\r\nSet-Cookie: malachimq_token=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0#{secure_flag}\r\nCache-Control: no-store\r\nContent-Length: 0\r\n\r\n"
  ...
  defp serve_logo(socket) do
    logo_path = Path.join(:code.priv_dir(:malachimq), "static/logo.jpeg")
  ...
  defp dashboard_html do
    ...
    <body>
      <div class="dashboard-header">
        <h1><img src="/logo.jpeg" alt="MalachiMQ" style="height: 42px; vertical-align: middle; margin-right: 10px;">MalachiMQ Dashboard</h1>
  ...
  defp login_page_html do
    ...
    <body>
      <div class="login-container">
        <h1><img src="/logo.jpeg" alt="MalachiMQ" style="height: 48px; vertical-align: middle; margin-right: 10px;">MalachiMQ</h1>
