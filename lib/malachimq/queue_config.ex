defmodule MalachiMQ.QueueConfig do
  ...
  def create_queue(queue_name, opts \\ []) do
    ...
    max_queues = Application.get_env(:malachimq, :max_dynamic_queues, 10_000)
  ...
  defp perform_update(queue_name, current_config, opts, new_max_buffer_size, force) do
    ...
    if new_max_buffer_size && new_max_buffer_size != current_config.max_buffer_size do
      ...
      threshold = Application.get_env(:malachimq, :update_excess_threshold, 0.5)
  ...
  defp get_current_buffer_size(queue_name) do
    case MalachiMQ.Queue.get_stats(queue_name) do
      %{buffered: size} -> size
      _ -> 0
    end
  end
  ...
  defp notify_config_updated(queue_name, new_config) do
    ...
    partition_count = Application.get_env(:malachimq, :partition_multiplier, 100) * System.schedulers_online()

    Enum.each(0..(partition_count - 1), fn partition ->
      # Use via_tuple to get the queue process pid
      via = {:via, Registry, {MalachiMQ.QueueRegistry, {queue_name, partition}}}
  ...
  defp get_default_max_message_size do
    Application.get_env(:malachimq, :default_max_message_size_bytes, 1_048_576)
  end

  defp get_default_max_buffer_size do
    Application.get_env(:malachimq, :default_max_buffer_size, 10_000)
  end

  defp get_default_overflow_behavior do
    behavior_str = Application.get_env(:malachimq, :default_overflow_behavior, "drop_newest")
  ...
  defp get_default_backpressure_threshold do
    Application.get_env(:malachimq, :default_backpressure_threshold, 0.8)
  end

  defp get_default_block_timeout do
    Application.get_env(:malachimq, :default_block_timeout_ms, 5_000)
  end

  defp get_default_max_blocked_producers do
    Application.get_env(:malachimq, :default_max_blocked_producers, 1_000)
  end
