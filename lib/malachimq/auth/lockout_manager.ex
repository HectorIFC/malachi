defmodule MalachiMQ.Auth.LockoutManager do
  @moduledoc """
  ...
  @table_attempts :malachimq_failed_attempts
  @table_lockouts :malachimq_locked_accounts
  # 1 minute
  ...
  def init(_opts) do
    ...
    Logger.info(MalachiMQ.I18n.t(:lockout_manager_started))
  ...
  def handle_cast({:failed_attempt, username, ip}, state) do
    key = {username, format_ip(ip)}
    now = System.system_time(:millisecond)
    max_attempts = Application.get_env(:malachimq, :max_auth_attempts, 5)
    ...
    MalachiMQ.Metrics.increment_failed_auth_attempt()
  ...
  defp apply_lockout(username, ip, attempt_count, state) do
    ...
    # Increments metric
    MalachiMQ.Metrics.increment_account_lockout()
  ...
  defp calculate_lockout_duration(attempt_count) do
    base_duration = Application.get_env(:malachimq, :lockout_duration_ms, 300_000)
    progressive = Application.get_env(:malachimq, :progressive_lockout, true)
    max_attempts = Application.get_env(:malachimq, :max_auth_attempts, 5)
