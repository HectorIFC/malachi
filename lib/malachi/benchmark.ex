defmodule Malachi.Benchmark do
  @moduledoc """
  ...
  def spawn_consumers(queue_name, count, callback \\ nil) do
    callback =
      callback ||
        fn msg ->
          log_every = Application.get_env(:malachi, :benchmark_log_every, 100_000)
    ...
    max_concurrency = Application.get_env(:malachi, :spawn_concurrency, 1000)
    ...
    _tasks =
      ...
      |> Task.async_stream(
        fn i ->
          opts = [hibernate: true]
          {:ok, _pid} = Malachi.Consumer.start_link({queue_name, callback, opts})
          log_every = Application.get_env(:malachi, :benchmark_log_every, 10_000)
  ...
  def send_messages(queue_name, count) do
    ...
    start_time = System.monotonic_time(:millisecond)
    max_concurrency = Application.get_env(:malachi, :send_concurrency, 1000)
