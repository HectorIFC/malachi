defmodule Malachi.AtomMonitor do
  @moduledoc """
  ...
  @impl true
  def init(_opts) do
    check_interval = Application.get_env(:malachi, :atom_check_interval_ms, 60_000)
    warning_threshold = Application.get_env(:malachi, :atom_warning_threshold, 0.7)
    critical_threshold = Application.get_env(:malachi, :atom_critical_threshold, 0.9)
  ...
  defp get_status(atom_count) do
    ...
    warning = Application.get_env(:malachi, :atom_warning_threshold, 0.7)
    critical = Application.get_env(:malachi, :atom_critical_threshold, 0.9)
  ...
  defp try_audit_log(event_type, context, action, status, metadata) do
    if Process.whereis(Malachi.AuditLog) do
      Malachi.AuditLog.log_event(event_type, context, action, status, metadata)
    end
  rescue
    _ -> :ok
  end
