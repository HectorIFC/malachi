defmodule Malachi.AckManager do
  @moduledoc """
  ...
  def track_message(message_id, queue_name, consumer_pid, message) do
    timeout = Application.get_env(:malachi, :ack_timeout_ms, @default_timeout_ms)
  ...
  def ack(message_id) do
    case :ets.lookup(@pending_table, message_id) do
      [{^message_id, entry}] ->
        ...
        Malachi.Metrics.increment_acked(entry.queue_name)
        :ok
  ...
  def nack(message_id, opts \\ []) do
    ...
    case :ets.lookup(@pending_table, message_id) do
      [{^message_id, entry}] ->
        :ets.delete(@pending_table, message_id)
        Malachi.Metrics.increment_nacked(entry.queue_name)

        if requeue do
          Malachi.Queue.enqueue(
            entry.queue_name,
  ...
  defp schedule_timeout_check do
    interval = Application.get_env(:malachi, :ack_check_interval_ms, 5_000)
    Process.send_after(self(), :check_timeouts, interval)
  end

  defp check_expired_messages do
    now = System.monotonic_time(:millisecond)
    max_retries = Application.get_env(:malachi, :max_retries, 3)
    ...
    Enum.each(expired, fn {message_id, entry} ->
      ...
      if entry.attempts < max_retries do
        ...
        Malachi.Metrics.increment_retried(entry.queue_name)
      else
        Logger.error(I18n.t(:message_failed_dlq, id: message_id, max: max_retries))

        Malachi.Metrics.increment_dead_lettered(entry.queue_name)
      end
    end)
