defmodule Malachi.Auth.LockoutServer do
  @moduledoc """
  A thin named facade over `Malachi.Cluster.RaCluster` for the cluster's account-lockout store (`Malachi.Auth.LockoutMachine`): start
  ...
  **Reads** (`locked?`/`failed_attempts`/`list_locked`) use `:ra.local_query` against the **local** replica:
  ...
  def start(cluster_name, nodes \\ [node()]) do
    RaCluster.start(LockoutMachine, cluster_name, nodes)
  end
  ...
  def reconcile(cluster_name, nodes), do: RaCluster.reconcile(LockoutMachine, cluster_name, nodes)
