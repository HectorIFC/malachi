defmodule Malachi.Cluster.LeaseMachine do
  ...
  raising here: `Lease.apply/3` has no catch-all clause.
  """
  ...
  @impl true
  def apply(meta, command, %Lease{} = state) do
    MachineVersion.apply(meta, command, state, Lease.command_versions(), fn meta, command, state ->
      Lease.apply(state, command, meta.system_time)
    end)
  end
end
