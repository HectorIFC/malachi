defmodule Malachi.Storage.ElixirStore do
  ...
  def sync(%__MODULE__{pending_count: 0} = store) do
    ...
    # pwrite in the clause below, which is followed by its fsync in the same breath, so no
  ...
  def sync(%__MODULE__{} = store) do
    ...
    # A full fsync, and measured to be the right call rather than assumed. fdatasync(2) skips the
    # mtime/atime journalling that fsync pays, so it looks like free throughput, but the paired
    # experiment in benchmark/storage_viability.exs (SYNC_AB=1, 15 interleaved repetitions on an
    # ext4 CI runner) found no difference: 341us against 345us at p50, where the fsync-vs-fsync
    # control differed by 3us. The reason is that a segment GROWS, so every append changes the file
    # size, and that size change is metadata fdatasync must journal anyway; what it saves rides
    # along in a commit it has to make regardless. Segment preallocation (issue #83) is what
    # removes the size change, and only after that does fdatasync have anything left to skip.
